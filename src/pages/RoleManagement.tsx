...
const RoleManagement: React.FC = () => {
  ...
  return (
    <div>
      <h1 style={{ textAlign: 'center', margin: '2rem 0', fontSize: '2.5rem', color: '#256541' }}>Role Management</h1>
      <div style={{ background: '#eaf6ef', padding: '2rem', borderRadius: 16, maxWidth: 1000, margin: '2rem auto', boxShadow: '0 2px 12px rgba(38,101,65,0.10)' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: '1.1rem' }}>
          <thead>
            <tr style={{ background: '#256541', color: '#fff' }}>
              ...
              <th style={{ padding: 12, borderTopRightRadius: 8 }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {paginatedUsers.map(user => {
              ...
              return (
                <tr key={user.uid} style={{ background: '#fff', borderBottom: '1px solid #d6e5db' }}>
                  <td style={{ padding: 10 }}>{user.uid}</td>
                  <td style={{ padding: 10 }}>{user.email}</td>
                  <td style={{ padding: 10 }}>{user.displayName}</td>
                  <td style={{ padding: 10 }}>
                    <select
                  ...
                  <td style={{ padding: 10 }}>
                    {isEditing && (
                      <>
                        <button
                          style={{ background: '#256541', color: '#fff', padding: '0.5rem 1.2rem', border: 'none', borderRadius: 6, marginRight: 8, cursor: 'pointer', fontWeight: 600 }}
                        ...
                        <button
                          style={{ background: '#eaf6ef', color: '#256541', padding: '0.5rem 1.2rem', border: '1px solid #256541', borderRadius: 6, cursor: 'pointer', fontWeight: 600 }}
